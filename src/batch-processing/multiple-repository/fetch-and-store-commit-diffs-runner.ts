import { fetchAndStoreMultipleRepositoryCommitDiffs } from './fetch-and-store-commit-diffs';
...
const main = async () => {
    try {
        ...
        console.log(`Fetching commit diffs from all ${USERNAME} repositories since ${startingDate.toISOString()}`);

        // Create GitHub API client with authentication
        const octokit = new Octokit({ auth: ALL_GITHUB_TOKEN });

        // Create commit diff retriever with dependency injection
        const commitDiffRetriever = new MultipleRepositoryGithubCommitDiffSupplier(
            ALL_GITHUB_TOKEN,
            USERNAME,
            RATE_LIMIT_DELAY,
            octokit
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
    console.log(`
Usage: node fetch-and-store-commit-diffs-runner.js [starting-date]
...
Examples:
  node fetch-and-store-commit-diffs-runner.js 2024-01-01
  node fetch-and-store-commit-diffs-runner.js 2024-10-01
  node fetch-and-store-commit-diffs-runner.js
...
Environment Variables:
  ...
  BATCH_SIZE         Optional. Number of diffs to process in batches (default: 50)
`);
    process.exit(0);
}
